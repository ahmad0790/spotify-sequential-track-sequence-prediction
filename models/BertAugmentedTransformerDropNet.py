...
if DROP_NET:
	from models.CustomizedTransformerDropNet import Transformer, TransformerEncoder, TransformerEncoderLayer, TransformerDecoder, TransformerDecoderLayer
else:	
...
class BertAugmentedTransformer(nn.Module):
	...
	def __init__(self, vocab_size, d_model, nhead, num_encoder_layers, num_decoder_layers, dim_feedforward, max_seq_length, skip_pred = False, feat_embed=None, device=None, p_net =None):
		...
		print(self.d_model_combined)

		self.layer_norm = nn.LayerNorm(self.d_model_combined)

		self.encoder_layer = TransformerEncoderLayer(d_model=self.d_model_combined, nhead=self.nhead, dropout=0.2, activation = 'relu', p_net = self.p_net)
		self.transformer_encoder = TransformerEncoder(self.encoder_layer, self.num_encoder_layers, norm= self.layer_norm)
		
		self.decoder_layer = TransformerDecoderLayer(d_model=self.d_model_combined, nhead=self.nhead, dropout = 0.2, activation= 'relu', p_net = self.p_net)
	...
	def forward(self, src, tgt, bert_output, src_skip_inputs=None, src_mask=None, tgt_mask=None, encoder_mask=None, train_flag=True):
		...
		encoder_output = self.encode(src, bert_output, train_flag=train_flag)
		decoder_output = self.decode(encoder_output, tgt, bert_output, train_flag=train_flag)
		output = self.fc(decoder_output)
		output = rearrange(output, 't n e -> n t e')
		return output
	
	def encode(self, src, src_bert, src_skip_inputs=None, src_mask=None, train_flag=True):
		source_embeddings = self.embed(src, src_skip_inputs)
		source_embeddings = rearrange(source_embeddings, 'n s t -> s n t')
		x = self.transformer_encoder(source_embeddings, src_bert, train_flag=train_flag)
		return x
	
	def decode(self, encoder_output, tgt, tgt_bert, src_mask=None, tgt_mask=None, encoder_mask = None, train_flag = True):
		...
		x = self.transformer_decoder(tgt = tgt_embeddings, tgt_bert = tgt_bert, memory = encoder_output, tgt_mask=tgt_no_peek_mask, train_flag=train_flag)
		return x
	...
	def greedy_decoder(self, model, src, bert_output, max_len, start_tgt_sequence):
		encoder_output = model.encode(src, bert_output, train_flag=False)
		...
		for i in range(max_len):
			...
			#tgt_no_peek_mask = self.gen_nopeek_mask(tgt_tokens.shape[1]).to(self.device)
			output = model.decode(encoder_output, tgt_tokens, bert_output, train_flag=False)
