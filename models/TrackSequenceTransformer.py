...
class StandardTransformer(nn.Module):
	...
	def forward(self, src, tgt, verbose=False):
		...
		if self.feat_embed:
			...
			source_seq_embeddings = torch.cat((self.embed_src(src), self.embed_feat(src)),2) + positional_embeddings
			...
			source_seq_embeddings = self.embed_src(src) + positional_embeddings
			target_sequence_embeddings = self.embed_tgt(tgt) + tgt_positional_embeddings
